...
#define W5500_RST GPIO_Pin_25   //W5500的复位引脚

//W5500网络参数结构体
typedef struct
{
	uint8_t s0_ModeSet;//端口0的运行模式
	
	uint8_t Gateway_IP0;//网关参数设置
	uint8_t Gateway_IP1;
	uint8_t Gateway_IP2;
	uint8_t Gateway_IP3;
	
	uint8_t Sub_Mask0;//加载子网掩码
	uint8_t Sub_Mask1;
	uint8_t Sub_Mask2;
	uint8_t Sub_Mask3;
	
	uint8_t Phy_Addr0;//加载设备物理地址
	uint8_t Phy_Addr1;
	uint8_t Phy_Addr2;
	uint8_t Phy_Addr3;
	uint8_t Phy_Addr4;
	...
	uint8_t IP_Addr0;//加载设备IP地址
	uint8_t IP_Addr1;
	uint8_t IP_Addr2;
	uint8_t IP_Addr3;
	
	uint8_t S0_Port0;//设备端口号
	uint8_t S0_Port1;
	
	uint8_t S0_DIP0;//加载目的IP地址
	uint8_t S0_DIP1;
	uint8_t S0_DIP2;
	uint8_t S0_DIP3;
	
	uint8_t S0_DPort0;//目的端口号
	uint8_t S0_DPort1;
...
extern void Load_Net_Parameters(_Typedef_Internet * Intnet_set);
